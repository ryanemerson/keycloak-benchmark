...
kubectl -n ${NAMESPACE} rollout status deployment.apps/advanced-tidb-discovery
kubectl -n ${NAMESPACE} rollout status statefulset.apps/advanced-tidb-pd
kubectl -n ${NAMESPACE} rollout status statefulset.apps/advanced-tidb-tidb
kubectl -n ${NAMESPACE} rollout status statefulset.apps/advanced-tidb-tikv
...
get_pods_by_zone() {
  ...
  # --- Input Validation ---
  if [ -z "$namespace" ]; then
    echo "Error: Namespace not provided." >&2
    echo "Usage: get_pods_by_zone <namespace>" >&2
    return 1
  fi
  ...
  # --- Data Structures ---
  # Associative array to map zone names to a string of pod names
  declare -A zone_pods
  # Associative array to cache node-to-zone mappings to reduce API calls
  declare -A node_zone_cache

  echo "Fetching pods and nodes from namespace: '$namespace'..."

  # --- Main Logic ---
  # Get a list of pods and the nodes they run on. We process this list line by line.
  # The <(...) process substitution is used to feed the while loop without creating a subshell
  # that would prevent our associative arrays from being modified.
  while IFS= read -r line; do
    # Skip empty lines which can occur if there are no pods
    if [ -z "$line" ]; then
        continue
    fi
    ...
    # If the node is '<none>', the pod is likely pending and not scheduled yet.
    if [ "$node_name" == "<none>" ]; then
      zone="unscheduled"
    # Check our cache first to avoid unnecessary kubectl calls
    elif [ -n "${node_zone_cache[$node_name]}" ]; then
      zone=${node_zone_cache[$node_name]}
    else
      # If not in cache, query the node for its failure zone label and cache it.
      # The label 'topology.kubernetes.io/zone' is standard for cloud providers.
      zone=$(kubectl get node "$node_name" -o=jsonpath='{.metadata.labels.topology\.kubernetes\.io/zone}' 2>/dev/null)
      if [ -z "$zone" ]; then
        zone="zone-not-found"
      fi
      node_zone_cache["$node_name"]=$zone
    fi

    # Append the pod name to the list for its corresponding zone.
    zone_pods["$zone"]+="$pod_name "

  done < <(kubectl get pods -n "$namespace" -o=custom-columns=POD:.metadata.name,NODE:.spec.nodeName --no-headers)


  # --- Output Results ---
  if [ ${#zone_pods[@]} -eq 0 ]; then
      echo "No pods found in namespace '$namespace'."
      return 0
  fi

  echo "----------------------------------------"
  echo "Pods grouped by failure zone:"
  echo "----------------------------------------"

  for zone in "${!zone_pods[@]}"; do
    echo "Zone: $zone"
    # Trim trailing space and format pods into a list
    local pods_list=$(echo "${zone_pods[$zone]}" | sed 's/ $//' | tr ' ' '\n')
    echo "$pods_list" | awk '{print "  - " $1}'
    echo "" # Add a blank line for readability
  done
}
get_pods_by_zone tidb-cluster
