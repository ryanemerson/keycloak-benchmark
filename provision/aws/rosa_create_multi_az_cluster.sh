...
KUBE_CONTEXT_A=$(tofu output -raw cluster_a_kube_context)
KUBE_CONTEXT_B=$(tofu output -raw cluster_b_kube_context)

cd ${SCRIPT_DIR}/../opentofu/modules/multi-az/dependencies
tofu init
tofu workspace new ${CLUSTER_PREFIX} || true

TOFU_CMD="tofu apply -auto-approve \
  -var kube_context_a=${KUBE_CONTEXT_A} \
  -var kube_context_b=${KUBE_CONTEXT_B}"
${TOFU_CMD}
